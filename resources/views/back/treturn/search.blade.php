<tr class="tr{{$transactiondetail->id}}" datari="{{$transactiondetail->transaction_id}}" dataproduct="{{$product->id}}" dataquantity="{{$qty}}" dataprice="{{$price}}">
	<td>
		<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/product/' . $product->id)}}" style="color: blue;">
			{{$product->name}}
			{!!Form::hidden('transactiondetail[' . $transactiondetail->id . '][product]', $product->id)!!}
		</a>
	</td>
	<td style="text-align: right;">
		{{$qty}}
		{!!Form::hidden('transactiondetail[' . $transactiondetail->id . '][qty]', $qty)!!}
	</td>
	<td style="text-align: right;">
		{{number_format($price)}}
		{!!Form::hidden('transactiondetail[' . $transactiondetail->id . '][price]', $price)!!}
	</td>
	<td style="text-align: right;">
		{{number_format($price * $qty)}}
		{!!Form::hidden('transactiondetail[' . $transactiondetail->id . '][subtotal]', $price * $qty, ['class'=>'subtotal'])!!}
	</td>
...
<script type="text/javascript">
	$(function(){
		...
		$('.index-action-switch'+{{$transactiondetail->id}}+' .edit').click(function(e){
			e.preventDefault();
		...
		$('.index-action-switch'+{{$transactiondetail->id}}+' .delete').click(function(e){
			e.preventDefault();
