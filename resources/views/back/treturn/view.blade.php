...
@section('page_title')
	Return View
	<span>
		<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/dashboard')}}">Dashboard</a> / <a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/treturn')}}">Return</a> / <span>Return View</span>
	</span>
@endsection
		...
		<div class="page-item col-1">
			<div class="page-item-content">
				<h1 class="view-title">
					...
					@else
						<a class="view-button-item view-button-back" href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/treturn')}}"></a>
					@endif
					{{$return->no_nota}}
					<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/treturn/' . $return->id . '/edit')}}" class="view-button-item view-button-edit">
						Edit
					</a>
				</h1>
				...
				@if (file_exists(public_path() . '/usr/img/treturn/' . $return->id . '_' . Str::slug($return->name, '_') . '_thumb.jpg'))
					{!!HTML::image('usr/img/treturn/' . $return->id . '_' . Str::slug($return->name, '_') . '_thumb.jpg?lastmod=' . Str::random(5), '', ['class'=>'view-photo'])!!}
				@endif
				<div class="page-group">
					<div class="page-item col-1">
						...
						<div class="page-item-content view-item-content">
							<table class="view-detail-table">
								<tr>
									...
									<td>
										<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/transaction/' . $return->transaction_id)}}" style="color: blue;">
								...
								<tr>
									...
									<td>
										<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/customer/' . $return->customer_id)}}" style="color: blue;">
						...
						<div class="page-item-content view-item-content">
							<table class="index-table">
								...
								<?php
									...
									$gettransactiondetails = Transactiondetail::where('transaction_id', '=', $return->transaction_id)->get();
									foreach ($gettransactiondetails as $gettransactiondetail) {
										$gettransactiondetailids[] = $gettransactiondetail->id;
									}
								?>

								@if(isset($gettransactiondetailids))
									<?php
										$returndetails = Treturndetail::where('treturn_id', '=', $return->id)->whereIn('transactiondetail_id', $gettransactiondetailids)->get();
									?>
									@foreach ($returndetails as $returndetail)
										...
										<tr>
											...
											<td>
												<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/product/' . $returndetail->transactiondetail->product_id)}}" style="color: blue;">
													{{$returndetail->transactiondetail->product->name}}
												</a>
											</td>
											<td style="color: green;">
												{{$returndetail->transactiondetail->qty}}
											</td>
											<td style="color: red;">
												{{$returndetail->qty}}
											</td>
